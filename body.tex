...
We used Wireshark to capture all Ethernet traffic traversing the access point, divided these data using MAC addresses into streams of packets corresponding to each IoT device, and saved the packets in PCAP files for offline analysis. 
 ...
 %Software patches because some of the most valuable information obtainable by an adversary would be a patch because it could reveal information about the way the embedded system in the device is designed. 
...
%The first approach used to determine whether a packet was cleartext or encrypted was the naive ASCII approach. 
...
\begin{figure}[t]
  \centering
    \fbox{\includegraphics[scale=0.45]{bloodpressure}}
